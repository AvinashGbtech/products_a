...
class ProductController extends CI_Controller {
	...
	public function imgCount($product_id=''){
		return $imgs = $this->db->where("product_id",$product_id)->get("product_imgs")->num_rows();
	}
...
//				if ()
	...
	public function uploadProductImg(){
	    ...
	    for ($i = 0; $i < $count; $i++) {
		        ...
		        if(move_uploaded_file($filename['tmp_name'],$location)){
		            ...
		            $this->db->insert("product_imgs",$imgData);
	            // echo $location;
		            $uploadedFiles++;
		        }
		}
	   ...
	   //          );
	   //          $this->db->insert("product_imgs",$data);
	 //            $uploadedFiles++;
		// 	// }
		// }
	...
	$response = array();
		$count = 0;
		$count1 = 0;
		$imgs = $this->db->where("product_id",$this->input->post("product_id"))->get("product_imgs")->result();
